 ...
 public:
  ...
  inline Status InsertFull(
    ...
    auto helper = lowest_used_cache_tier > CacheTier::kVolatileTier
                      ? GetFullHelper()
                      : GetBasicHelper();
    // Cache::CacheItemHelper* helper_ptr = const_cast<Cache::CacheItemHelper*>(
    //     reinterpret_cast<const Cache::CacheItemHelper*>(helper));
    // helper_ptr->level = level;

    // Cache::CacheItemHelper* helper_ptr = new Cache::CacheItemHelper();
    // helper_ptr->level = level;
    // helper_ptr->need_free = true;

    // return this->cache_->Insert(key, UpCastValue(value), helper_ptr, charge,
