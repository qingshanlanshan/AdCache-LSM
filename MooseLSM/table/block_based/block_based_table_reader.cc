...
Status ReadAndParseBlockFromFile(
  ...
  {
    auto ro = const_cast<ReadOptions&>(options);
    ro.extern_options->n_IO++;
  }
  if (s.ok()) {
  ...
  if (block_cache) {
    ...
    if (out_parsed_block->GetValue() == nullptr &&
      ...
      if (!contents) {
        ...
        ExternOptions* extern_options = const_cast <ExternOptions*>(ro.extern_options);
        if (extern_options != nullptr)
          extern_options->n_IO++;
