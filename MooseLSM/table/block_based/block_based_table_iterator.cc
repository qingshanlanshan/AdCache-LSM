...
inline Slice get_next_key(const Slice& key)
{
  assert(key.size() == 24);
  auto next_key = key.ToString();
  next_key[next_key.size() - 8 - 1] = next_key[next_key.size() - 8 - 1] + 1;
  return Slice(next_key);
}
...
void BlockBasedTableIterator::InitDataBlock() {
  ...
  if (!block_iter_points_to_real_block_ ||
    ...
    if (is_in_cache) {
      ...
      bool use_rl_cache = false;
      auto block_cache = rep->table_options.block_cache.get();
      ...
      {
        int n_IO = ro.extern_options->n_IO;
        if (block_miss_counter > 5 && block_miss_counter > block_hit_counter)
          ro.fill_cache = false;
        table_->NewDataBlockIterator<DataBlockIter>(
            ro, data_block_handle, &block_iter_, BlockType::kData,
            /*get_context=*/nullptr, &lookup_context_,
            block_prefetcher_.prefetch_buffer(),
            /*for_compaction=*/is_for_compaction, /*async_read=*/false, s,
            use_block_cache_for_lookup);
        if (ro.extern_options->n_IO != n_IO) {
          assert(ro.extern_options->n_IO == n_IO + 1);
          block_miss_counter++;
        }
        else {
          block_hit_counter++;
