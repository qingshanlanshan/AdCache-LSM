...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
  ...
  // shortcut for heap cache
  bool use_heap_cache = false;
  TableReader* t = f ? f->file_metadata->fd.table_reader : nullptr;
  BlockBasedTable* block_table = nullptr;
  Cache* block_cache = nullptr;
  if (t)
  {
    block_table = dynamic_cast<BlockBasedTable*>(t);
    if (block_table)
    {
      auto rep = block_table->get_rep();
  ...
  // use_heap_cache = false;
  if (use_heap_cache)
  {
    // Cache::CacheItemHelper item_helper = *GetCacheItemHelper(BlockType::kData);
    // item_helper.type = Cache::CacheItemHelper::CacheItemType::kCacheKV;
    Cache::CacheItemHelper item_helper;
    item_helper.type = Cache::CacheItemHelper::CacheItemType::kCacheKV;
    Cache::Handle* handle = block_cache->Lookup(user_key, &item_helper);
    if (handle)
    {
      std::string* v = reinterpret_cast<std::string*>(block_cache->Value(handle));
      PinnableSlice pinnable_value(v);
      *value = std::move(pinnable_value);
      *status = Status::OK();
      return;
    }
  }
  ExternOptions* extern_options = const_cast <ExternOptions*>(read_options.extern_options);
  LevelCache* level_cache = extern_options ? extern_options->level_cache : nullptr;
  ...
  while (f != nullptr) {
    if (level_cache)
    {
      ...
      std::string s;
      if (level_cache->Get(user_key.ToString(), s, level))
      {
        PinnableSlice pinnable_value(&s);
        *value = std::move(pinnable_value);
        *status = Status::OK();
        return;
      }
    ...
    auto ro = read_options;
    if (use_heap_cache)
    {
      if (block_cache->GetUsage() > block_cache->GetCapacity() * 0.9)
        ro.fill_cache = false;
    }
    *status = table_cache_->Get(
        ro, *internal_comparator(), *f->file_metadata, ikey,
    ...
    switch (get_context.State()) {
      ...
      case GetContext::kFound:
        if (extern_options)
          extern_options->hit_level = fp.GetHitFileLevel();
        ...
        if (use_heap_cache)
        {
          ...
          // Cache::CacheItemHelper item_helper = *GetCacheItemHelper(BlockType::kData);
          Cache::CacheItemHelper *item_helper = new Cache::CacheItemHelper();
          item_helper->cost = extern_options->n_IO;
          item_helper->type = Cache::CacheItemHelper::CacheItemType::kCacheKV;
          item_helper->level = fp.GetHitFileLevel();
          item_helper->need_free = true;
          block_cache->Insert(user_key, &s, item_helper, s.size()+user_key.size());
        }
        // if (level_cache)
        // {
        //   level_cache->insert(fp.GetHitFileLevel(), ikey.ToString(), value->ToString());
        // }
        return;
...
bool VersionStorageInfo::EnableDynamicRun(int logical_level, const MutableCFOptions& options) {
  assert(logical_level < (int)options.run_numbers.size());
