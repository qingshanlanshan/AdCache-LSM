...
Status TableCache::Get(
  ...
  if (!done) {
    ...
    if (s.ok()) {
      ...
      if (options.extern_options && options.extern_options->level_cache) {
        auto level_cache = options.extern_options->level_cache;
        // get kv pairs from t
        std::vector<std::pair<std::string, std::string>> range;
        InternalIterator* iter = t->NewIterator(options, prefix_extractor.get(), nullptr,
                                        true, TableReaderCaller::kUserGet);
        iter->SeekToFirst();
        for (int i = 0; iter->Valid() && i < 4; iter->Next(), ++i) {
          range.push_back({iter->key().ToString(), iter->value().ToString()});
        }
        level_cache->insert_range(level, range);
        delete iter;
      }
