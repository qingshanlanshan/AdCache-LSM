...
Compaction* MooseCompactionPicker::PickCompaction(
  ...
  MooseCompactionBuilder builder(cf_name, vstorage, this, log_buffer,
                                 ...
                                 mutable_db_options);
  Compaction* res = builder.PickCompaction();
  // do prefetching
  // if (res != nullptr && mutable_cf_options.extern_options->cache_style == "RLCache") {
  //   auto inputs = *res->inputs();
  //   auto block_cache = mutable_cf_options.extern_options->block_cache;
  //   if (block_cache)
  //   {
  //     size_t blockcache_size = block_cache->GetCapacity();
  //     size_t prefetch_size = 0;
  //     for (auto& input : inputs) {
  //       for (auto filemetadata : input.files) {
  //         filemetadata->fd.table_reader->Prefetch(ReadOptions());
  //         // sst size = 4MB
  //         prefetch_size += 4 << 20;
  //         if (prefetch_size + (4 << 20) > blockcache_size) {
  //           break;
  //         }
  //       }
  //     }
