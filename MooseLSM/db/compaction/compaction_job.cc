...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  (void)use_heap_cache;  // avoid unused variable warning
  Cache* block_cache = nullptr;
  auto extern_options = cfd->GetCurrentMutableCFOptions()->extern_options;
  if (extern_options && extern_options->block_cache) {
    block_cache = extern_options->block_cache.get();
  }
  if (block_cache) {
    std::string cache_name = block_cache->Name();
    use_heap_cache = (cache_name == "HeapCache");
    block_cache->Lookup("compact");
  }
  ...
  if (extern_options && extern_options->level_cache)
  {
    auto level_cache = extern_options->level_cache;
    int start_level = sub_compact->compaction->start_level();
    int output_level = sub_compact->compaction->output_level();
    string start_string = start_slice.ToString();
    string end_string = end_slice.ToString();
    for (int level = start_level; level < output_level; level++)
    {
      level_cache->merge(level, output_level, &start_string, &end_string);
    }
  }
  if (use_heap_cache) {
    block_cache->Lookup("compact_done");
  }
}
